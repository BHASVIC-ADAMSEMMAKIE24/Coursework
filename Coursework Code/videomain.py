...
# --------- CARD IMAGE LOADING (NO DICTS, USE LIST OF [CODE, IMAGE]) ----------

card_image_list = []  # each entry: [card_code, surface]
...
def draw_blackjack_placeholder(surface):
    ...
    if bj_phase == BJ_PHASE_SELECT_PLAYERS:
        ...
        range_text = "Min: " + str(bj_slider_min_value) + "   Max: " + str(bj_slider_max_value) + "   Current: " + str(bj_slider_value)
...
def draw_rules_placeholder(surface):
    ...
    # Instructions (top-right, same style as Blackjack screen)
    info_surf = FONT.render("Press M to return to Menu", True, (0, 0, 0))
    info_rect = info_surf.get_rect(topright=(1260, 20))
    surface.blit(info_surf, info_rect)

    # Text blocks (same content as Rules() prints)
    blackjack_text = (
        "Blackjack Rules:\n"
    ...
    # Helper to draw multi-line blocks on the left side
    ...
    # Draw hand rankings (might extend further down the screen)
    ...
    for event in pygame.event.get():
        ...
        if current_state == STATE_MENU:
            ...
            if card_menu_option_is_clicked(poker_option, event):
                click_effect.play()
                current_state = STATE_BLACKJACK  # placeholder
